    ...
    for (const part of parts) {
      if (part.includes("-")) {
        ...
        if (start >= 1 && end <= totalPages && start <= end) {
          validRanges.push([start - 1, end - 1]) // 0-based indexing
        }
      } else {
        const page = parseInt(part, 10)
        if (page >= 1 && page <= totalPages) {
          validRanges.push([page - 1, page - 1])
        }
      }
    }
    ...
    try {
      ...
      if (splitMode === "pages") {
        ...
        for (let i = 0; i < totalPages; i++) {
          ...
          a.href = url
          a.download = `${pdfFile.name.replace(/\.pdf$/i, "")}_page_${i + 1}.pdf`
        ...
        for (let i = 0; i < ranges.length; i++) {
          ...
          const a = document.createElement("a")
          a.href = url
          a.download = `${pdfFile.name.replace(/\.pdf$/i, "")}_pages_${start + 1}-${end + 1}.pdf`
          document.body.appendChild(a)
          a.click()
          document.body.removeChild(a)
          URL.revokeObjectURL(url)
                ...
                {isSplitting && (
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-muted-foreground text-center">
                      Processing {pdfFile?.pageCount} pages...
                    </p>
                  </div>
                )}
              </CardContent>
